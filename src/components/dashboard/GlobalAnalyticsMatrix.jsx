...
<div className="bg-[#111827] border border-slate-800 rounded-xl p-6 print:bg-white print:border-slate-300 print:p-2 print:text-black page-break-inside-avoid break-inside-avoid">
  
  {/* CRITICAL PRINT REPAIR INJECTION: Forces full width scaling and structured cell breaks on paper */}
  <style dangerouslySetInnerHTML={{__html: `
    @media print {
      ...
      /* Maintain risk contrast weights on white paper background */
      table.print-matrix-force td.bg-red-900\\/70 { background-color: #fee2e2 !important; color: #991b1b !important; font-weight: 900 !important; }
      table.print-matrix-force td.bg-amber-900\\/60 { background-color: #fef3c7 !important; color: #92400e !important; font-weight: 700 !important; }
      table.print-matrix-force td.bg-blue-950\\/50 { background-color: #eff6ff !important; color: #1e40af !important; }
  ...
  <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4 print:border-slate-300">
    <div className="flex items-center gap-2">
      <Grid size={16} className="text-red-400 print:text-red-700" /> 
  ...
  <div className="mb-4 bg-[#0b0f19] border border-slate-800 p-3 rounded-lg flex flex-wrap items-center gap-6 text-[10px] font-mono print:bg-white print:border-slate-200">
    ...
    <div className="flex items-center gap-2">
      <div className="w-2.5 h-2.5 bg-slate-950 border border-slate-800 rounded print:border-slate-300"></div>
      <span className="text-slate-300 font-black print:text-slate-700">Clear / Baseline Line ($0)</span>
    </div>
    <div className="flex items-center gap-2">
      <div className="w-2.5 h-2.5 bg-blue-950/50 border border-blue-900/50 rounded print:bg-blue-100 print:border-blue-300"></div>
      <span className="text-blue-300 font-black print:text-blue-800">Low Value Fragmented Diversion (&lt;$1k)</span>
    </div>
    <div className="flex items-center gap-2">
      ...
      <span className="text-amber-200 font-black print:text-amber-800">Transshipment Bypass Activity ($1k–$2k)</span>
    </div>
    <div className="flex items-center gap-2">
      <div className="w-2.5 h-2.5 bg-red-900/70 border border-red-800/60 rounded print:bg-red-100 print:border-red-300"></div>
      <span className="text-red-200 font-black print:text-red-800">Critical Value Displacement Volatility (&gt;$3k)</span>
    </div>
  </div>

  {/* High-Contrast Grid Layout Container with print helper class */}
  <div className="overflow-x-auto bg-[#0b0f19] p-5 rounded-xl border border-slate-900 print-matrix-container">
    <table className="w-full text-left font-mono text-[11px] border-collapse min-w-[750px] print-matrix-force">
      <thead>
        <tr className="bg-[#090d16] print:bg-slate-100">
          <th className="p-3 border border-slate-800 text-slate-200 font-black uppercase text-[10px] tracking-wider print:border-slate-300 print:text-slate-900">ORIGIN \ DEST</th>
          {advancedMetrics.destinations.map(dst => (
            <th key={dst} className="p-3 border border-slate-800 text-slate-200 font-black uppercase tracking-tight text-center max-w-[140px] leading-tight text-[10px] print:border-slate-300 print:text-slate-900">
              {dst}
            </th>
          ))}
        </tr>
  ...
  {selectedCell && (
    <div className="mt-4 p-4 bg-[#0a0f1d] border border-red-900/50 rounded-xl space-y-3 animate-fadeIn print:bg-white print:border-slate-300 print:mt-6">
      ...
      {/* Container class switches from scrollbar behavior to unrolled multi-column display during printing */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[250px] overflow-y-auto pr-1 print-expand-ledger">
        {filteredCellRecords.map((row, i) => (
          <div key={i} className="bg-[#111827] border border-slate-800 p-4 rounded-xl flex flex-col justify-between space-y-3 print-ledger-card print:bg-slate-50">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-emerald-400 font-black font-mono print:text-emerald-800">
  ...
  <div className="mt-4 p-4 bg-[#0f172a] rounded-lg border border-slate-800 font-mono text-xs text-slate-300 space-y-3 print:bg-white print:border-slate-300 print:text-slate-800">
    ...
    <div className="border-t border-slate-800/80 pt-2 text-[11px] text-slate-400 leading-relaxed space-y-1 print:border-slate-200 print:text-slate-600">
      ...
      <div>• <strong className="text-amber-400 uppercase print:text-amber-700">Transshipment Bypass Clusters (Amber Nodes):</strong> Classical bypass behavior where cargo swaps custom identifiers or documentation records inside intermediate Free Trade Zones.</div>
    </div>
  </div>
</div>
