...
export default function CountryRiskIntelligence() {
  ...
  // Broadened dynamic risk tiering engine (No longer discards standard data)
  const riskAnalysis = useMemo(() => {
    return tradeData.map((row, idx) => {
      ...
      // Dynamic interpretation of transshipment points
  ...
  // Safely extract coordinates/labels for the vector map visualizer
              ...
              <div className="w-full flex justify-between items-center max-w-2xl relative z-10">
                ...
                <div className="flex flex-col items-center text-center space-y-2">
                  ...
                  <span className="text-[11px] font-mono font-bold text-slate-400 block max-w-[140px] truncate print-text-muted">
                    {activeRouteForMap.riskType === 'STRATEGIC_COMMODITY_DIVERSION' ? 'FTZ TRANSIT HUB' : 'DIRECT CROSS-BORDER'}
                  </span>
                </div>
