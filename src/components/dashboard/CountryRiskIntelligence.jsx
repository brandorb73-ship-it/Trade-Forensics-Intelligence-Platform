...
import { Globe, ShieldAlert, FileText, Server, Info, ArrowRight, Share2, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export default function CountryRiskIntelligence() {
  ...
  // Broadened dynamic risk tiering engine
  const riskAnalysis = useMemo(() => {
    return tradeData.map((row, idx) => {
      if (!row) return null;

      const origin = (row.OriginCountry || '').toUpperCase().trim();
      const destination = (row.DestinationCountry || row.Destination || 'UNITED STATES').toUpperCase().trim();
      ...
      // Dynamic interpretation of transshipment points and actual destinations
      let routePath = hasRouteString ? origin : `${origin} → ${destination}`;
      let riskType = 'MONITORED_BASELINE';
      let severity = 'LOW';
      let brief = `Logistical routing falls within standard bilateral parameters. Direct shipping lane verified from ${origin} to ${destination}.`;

      // ISO & Text matching for global transshipment hubs
      const isHub = 
      ...
      if (isHub && isStrategic) {
        ...
        severity = 'HIGH';
        routePath = hasRouteString ? origin : `${origin} → [SINGAPORE/FTZ TRANSIT] → ${destination}`;
        brief = `High-risk corridor diversion vector identified. Critical raw components or controlled commodities moving through an unverified intermediary transshipment zone before reaching ${destination}.`;
      } else if (hasRouteString || isHub) {
        riskType = 'CIRCUMVENTION_RISK';
        severity = 'MEDIUM';
        routePath = hasRouteString ? origin : `${origin} → [FTZ LOOP] → ${destination}`;
        brief = `Multi-jurisdictional route optimization or potential origin-switching detected. Logistics pattern utilizes transshipment hubs prior to entry into ${destination}.`;
      }
  ...
  // Aligns visual map with the summary analysis by floating higher density origin corridors to the top
  const filtered = useMemo(() => {
    const counts = {};
    filteredSubset.forEach(item => {
      counts[item.cleanOrigin] = (counts[item.cleanOrigin] || 0) + 1;
    });
    return [...filteredSubset].sort((a, b) => counts[b.cleanOrigin] - counts[a.cleanOrigin]);
  }, [filteredSubset]);
  ...
  const structuralInsights = useMemo(() => {
    ...
    const isIndustrialInput = topProduct.includes('ROD') || topProduct.includes('TOW') || topProduct.includes('TOBACCO');
    const cargoDescription = isIndustrialInput ? 'critical manufacturing inputs / production precursors' : 'specialized / regulated commodities';
    ...
    const evidentiaryFinding = `Analysis confirms focused transaction volumes associated with ${uniqueTargets} unique target entities. Rather than declaring strict compliance failure, patterns suggest unverified channels bypassing traditional authorized infrastructure.`;
  ...
  // Extract variables for visual map alignment
    ...
    <div className="space-y-6 text-slate-100 id-print-section max-w-[1800px] mx-auto p-1">
      ...
      <div className="flex justify-between items-center border-b border-slate-800 pb-4 non-printable">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">Comprehensive structural analysis of multi-jurisdictional route splitting, customs transshipment hubs, and logistics discrepancies.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 print-break-avoid">
        <div className="bg-[#111827] border-l-4 border-blue-500 p-5 rounded-xl xl:col-span-2 space-y-3 print-border-clean">
          ...
          <p className="text-xs text-slate-100 font-mono leading-relaxed print-text-dark">
            <strong>Logistical Context:</strong> {structuralInsights.contextText}
          </p>
          <p className="text-xs text-slate-300 font-mono leading-relaxed print-text-muted">
        ...
        <div className="bg-[#111827] border border-slate-800 p-5 rounded-xl flex flex-col justify-center space-y-2 print-border-clean">
          ...
          <span className="text-[10px] font-mono text-slate-200 block border-t border-slate-800/60 pt-2 print-text-muted print-border-clean">
        ...
        {activeRouteForMap ? (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-center pt-2">
            ...
            <div className="lg:col-span-3 bg-slate-950/80 rounded-xl p-6 border border-slate-900 flex flex-col justify-center items-center relative min-h-[160px] overflow-hidden print-border-clean print-container-expand">
              ...
              <div className="w-full flex justify-between items-center max-w-2xl relative z-10">
                ...
                <div className="flex flex-col items-center text-center space-y-2">
                  ...
                  <span className="text-[11px] font-mono font-black text-white block max-w-[120px] truncate print-text-dark">
                ...
                <div className="flex-1 flex flex-col items-center mx-2 relative">
                  <span className="text-[9px] text-slate-300 font-mono mb-1 tracking-tighter uppercase font-bold non-printable">Transit Link</span>
                ...
                <div className="flex flex-col items-center text-center space-y-2">
                  ...
                  <span className="text-[11px] font-mono font-bold text-slate-200 block max-w-[140px] truncate print-text-muted">
                    {activeRouteForMap.riskType === 'STRATEGIC_COMMODITY_DIVERSION' ? 'FTZ TRANSIT HUB' : 'DIRECT SHIPPING'}
                  </span>
                </div>

                {/* Vector Flow Line 2 */}
                <div className="flex-1 flex flex-col items-center mx-2 relative">
                  <span className="text-[9px] text-slate-300 font-mono mb-1 tracking-tighter uppercase font-bold non-printable">Consignee Leg</span>
                ...
                <div className="flex flex-col items-center text-center space-y-2">
                  ...
                  <span className="text-[11px] font-mono font-black text-white block max-w-[120px] truncate print-text-dark">
                    {activeRouteForMap.cleanDestination}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-950/40 border border-slate-800/80 p-4 rounded-xl space-y-2 h-full flex flex-col justify-center print-border-clean">
              <span className="text-[9px] font-mono font-black text-slate-300 uppercase tracking-wider block">Audited Pathway Vector</span>
              <div className="text-xs font-mono font-bold text-slate-100 line-clamp-2 print-text-dark">
              ...
              <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between text-[10px] font-mono print-border-clean">
                <span className="text-slate-300 font-medium print-text-muted">Risk Severity:</span>
                <span className={`font-black uppercase ${
                  activeRouteForMap.severity === 'HIGH' ? 'text-amber-400' : activeRouteForMap.severity === 'MEDIUM' ? 'text-blue-400' : 'text-slate-300'
      ...
      <div className="bg-[#111827] border border-slate-800 rounded-xl p-5 shadow-lg space-y-4 print-break-avoid print-border-clean">
        <div className="flex items-center gap-2.5 border-b border-slate-800 pb-3 print-border-clean">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-300 font-mono print-text-muted">Dynamic algorithmic threat overview and supply chain verification matrix</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 print-container-expand">
        ...
        <div className="space-y-2 non-printable">
          ...
          {[
            ...
            { id: 'STRATEGIC_COMMODITY_DIVERSION', label: 'Strategic Cargo Diversion' },
            { id: 'CIRCUMVENTION_RISK', label: 'Customs Circumvention' },
            { id: 'MONITORED_BASELINE', label: 'Bilateral Baseline Lanes' }
          ].map(tab => (
            <button 
              key={tab.id} 
              onClick={() => {
            ...
            filtered.map((evt, idx) => (
              <div 
                ...
                className={`p-5 rounded-xl border bg-[#111827] cursor-pointer transition-all print-break-avoid print-border-clean ${
                  ...
                  evt.severity === 'HIGH' ? 'border-l-4 border-l-amber-500' : evt.severity === 'MEDIUM' ? 'border-l-4 border-l-blue-500' : 'border-l-4 border-l-slate-400'
                }`}
              >
                ...
                <div className="flex justify-between border-b border-slate-800/60 pb-2 mb-3 font-mono text-xs print-border-clean">
                  <span className={`font-black uppercase tracking-wider flex items-center gap-1.5 ${
                    evt.severity === 'HIGH' ? 'text-amber-400' : evt.severity === 'MEDIUM' ? 'text-blue-400' : 'text-slate-300'
                  } print-text-dark`}>
                  ...
                  <span className="text-slate-300 font-bold print-text-muted">{evt.Date || '2026 Audit Cycle'}</span>
                </div>

                {/* Audit Narrative block */}
                <div className="space-y-2 mb-4">
                  <p className="text-xs font-mono text-slate-100 leading-relaxed print-text-dark">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-800/40 text-xs font-mono print-border-clean print-container-expand">
                  <div>
                    <span className="block text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-0.5 print-text-muted">Reconstructed Sourcing Route Path</span>
                    <span className="text-white font-bold print-text-dark">{evt.routePath}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-0.5 print-text-muted">Commodity Description</span>
                    <span className="text-slate-100 truncate block max-w-xs print-text-dark">{evt.Product || 'Unclassified Item'}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-0.5 print-text-muted">Target Consignee (Entity Linkage)</span>
                ...
                <div className="mt-4 pt-3 border-t border-slate-800/20 text-[10px] font-mono text-slate-300 flex items-start gap-1.5 bg-[#0f172a]/40 p-2.5 rounded-lg border border-slate-800/40 print-border-clean">
                  <Info size={12} className="text-blue-400 mt-0.5 flex-shrink-0 non-printable" />
                  <div>
                    <strong className="text-slate-200 uppercase tracking-tight block mb-0.5 print-text-dark">Possible Litigation Relevance:</strong>
