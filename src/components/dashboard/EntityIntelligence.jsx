  ...
  return (
    <div className="p-6 space-y-6 max-w-[1800px] mx-auto id-print-section">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden lg:col-span-3">
                ...
                {filteredLinks.length > 0 ? (
                  filteredLinks.map((link, i) => (
                    <tr key={link.id || i} className={`hover:bg-slate-700/30 transition-colors ${link.isRisk ? 'bg-rose-950/10' : ''}`}>
                      ...
                      <td className="px-4 py-3 text-right font-medium text-slate-300">
                        {link.weight ? link.weight.toLocaleString() : '0'} <span className="text-[10px] text-slate-500">KG</span>
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-slate-100">
                        ${link.amount ? Number(link.amount).toLocaleString(undefined, { minimumFractionDigits: 2 }) : '0.00'}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {link.isRisk ? (
                          <span className="text-rose-400 bg-rose-950/40 px-2 py-0.5 rounded border border-rose-900 text-[10px] font-black tracking-wider">HIGH RISK</span>
                        ) : (
